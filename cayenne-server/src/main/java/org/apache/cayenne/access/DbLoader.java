...
literal|".*"
decl_stmt|;
comment|/**      * CAY-479 - need to track which entities which are skipped during loading from db since it it already present in      * dataMap and haven't marked for overriding so that relationships to non-skipped entities can be loaded      */
specifier|private
name|Set
argument_list|<
name|DbEntity
...
return|;
block|}
comment|/**      * Loads dbEntities for the specified tables.      * @param map      *            DataMap to be populated with DbEntities.      * @param tables      * @param config      */
specifier|public
name|List
argument_list|<
name|DbEntity
...
name|getPrimaryKeysForEachTableAndStoreItInDbEntity
argument_list|(
name|map
argument_list|,
name|tables
...
condition|)
block|{
comment|// TODO is it ok? equals is not overridden
continue|continue;
block|}
name|ResultSet
name|rs
...
try|try
block|{
comment|// these will be initialized every time a new target entity is found
comment|// in the result set (which should be ordered by table name among other things)
name|DbRelationship
...
condition|)
block|{
comment|// cay-479 - don't doLoad relationships between two skipped entities.
continue|continue;
block|}
else|else
block|{
...
return|;
block|}
comment|/**      * Performs database reverse engineering and generates DataMap object that      * contains default mapping of the tables and views. Allows to limit types      * of tables to read.      *       * @deprecated since 4.0 use      */
annotation|@
name|Deprecated
specifier|public
name|DataMap
