...
name|CayenneRuntimeException
argument_list|(
literal|"Incorrect path '%s' translation, embeddable expected"
argument_list|,
name|result
operator|.
name|getFinalPath
...
name|valueSnapshot
init|=
name|getEmbeddableValueSnapshot
argument_list|(
name|embeddable
argument_list|,
name|node
...
expr_stmt|;
return|return
name|buildMultiValueComparison
argument_list|(
name|result
argument_list|,
name|valueSnapshot
...
name|CayenneRuntimeException
argument_list|(
literal|"Incorrect path '%s' translation, relationship expected"
argument_list|,
name|result
operator|.
name|getFinalPath
...
name|CayenneRuntimeException
argument_list|(
literal|"Unsupported or incorrect mapping for relationship '%s.%s': "
operator|+
literal|"target entity has different count of primary keys than count of joins."
argument_list|,
name|relationship
operator|.
...
name|valueSnapshot
init|=
name|getMultiAttributeValueSnapshot
argument_list|(
name|node
argument_list|,
name|parentNode
argument_list|)
decl_stmt|;
comment|// convert snapshot if we have attributes from source, not target
if|if
condition|(
name|result
operator|.
...
expr_stmt|;
block|}
comment|// build compound PK/FK comparison node
name|Node
name|multiValueComparison
init|=
name|buildMultiValueComparison
argument_list|(
name|result
...
argument_list|)
decl_stmt|;
comment|// replace current node with multi value comparison
name|Node
name|currentNodeParent
init|=
name|currentNode
operator|.
name|getParent
...
name|currentNodeParent
expr_stmt|;
comment|// we should skip all related nodes as we build this part of the tree manually
name|expressionsToSkip
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// TODO: support comparison of multi attribute ObjPath with other multi attribute ObjPath
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|(
literal|"Comparison of multiple attributes not supported for ObjPath"
argument_list|)
throw|;
block|}
block|}
