...
argument_list|)
decl_stmt|;
comment|// merge the snapshots of the FAKs by ObjectId for all ToOne relationships in case we have multiple Arcs per Object
name|Map
argument_list|<
name|ObjectId
argument_list|,
...
argument_list|()
decl_stmt|;
comment|// gather the list of the ToMany snapshots (these will actually be their own insert rows)
name|List
argument_list|<
name|Map
argument_list|<
...
block|{
comment|// merge joinSnapshot data with existing snapshot
for|for
control|(
name|Map
operator|.
...
block|}
block|}
comment|// apply the merged ToOne snapshots information and possibly merge it with an existing BatchQueryRow
for|for
control|(
name|Map
operator|.
...
expr_stmt|;
block|}
comment|// apply the ToMany snapshots as new BatchQueryRows
for|for
control|(
name|Map
argument_list|<
