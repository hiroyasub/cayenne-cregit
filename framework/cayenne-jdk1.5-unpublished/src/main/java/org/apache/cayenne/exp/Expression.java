...
throw|;
block|}
comment|// Retrieve, if possible, the expression from the cache.
name|Expression
name|expression
init|=
name|expressionCache
...
argument_list|)
decl_stmt|;
comment|// If the expression was cached, return it immediately without parsing.
if|if
condition|(
name|expression
operator|!=
...
name|expression
return|;
comment|// Couldn't find expression, parse, cache, and return it.
try|try
block|{
name|Reader
name|reader
