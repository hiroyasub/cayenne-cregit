...
argument_list|,
operator|new
name|JpaSQLResultSetMappingVisitor
argument_list|()
argument_list|)
expr_stmt|;
return|return
...
specifier|private
name|String
name|getSecondaryTableDbRelationshipName
parameter_list|(
name|String
...
argument_list|)
expr_stmt|;
comment|// for each embedded attribute, add all Embeddable attributes to DbEntity,
comment|// honoring @Column settings
name|JpaEmbeddable
name|jpaEmbeddable
init|=
name|path
...
block|}
class|class
name|JpaEmbeddableBasicVisitor
extends|extends
name|NestedVisitor
block|{
annotation|@
...
name|path
argument_list|,
literal|"Unrecognized secondary table: '"
operator|+
name|column
operator|.
name|getTable
...
block|{
comment|// TODO: andrus, 12/23/2007 this would miss a case if a user decides to
comment|// specify a secondary table on an abstract superclass that is not linked
comment|// to a table yet ... which would be quite crazy, but still...
return|return
name|column
operator|.
name|getName
...
argument_list|)
expr_stmt|;
comment|// assuming id's can not be flattened to another table...
name|cayenneAttribute
operator|.
name|setDbAttributePath
argument_list|(
...
expr_stmt|;
block|}
comment|// TODO: andrus 2/23/2008 - discriminator column...
block|}
comment|// my understanding of JPA SQLResultSetMapping is that regardless of whether
comment|// it belongs to entity or to the entire persistence unit, the scope is still
comment|// persistence unit??
name|targetPath
operator|.
name|firstInstanceOf
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// TODO: andrus 8/6/2006 - handle EmbeddedId, AttributeOverride
name|addChildVisitor
argument_list|(
name|JpaAttributes
operator|.
...
name|jpaEntity
operator|.
name|lookupInheritanceStrategy
argument_list|()
operator|==
name|InheritanceType
operator|.
...
control|)
block|{
comment|// create a relationship between master DbEntity and a secondary
comment|// DbEntity...
name|DbEntity
...
name|path
argument_list|,
literal|"Invalid referenced column name: "
operator|+
name|column
operator|.
name|getReferencedColumnName
...
expr_stmt|;
block|}
comment|// init discriminator column
name|JpaDiscriminatorColumn
name|discriminator
init|=
name|entity
operator|.
name|lookupDiscriminatorColumn
argument_list|()
decl_stmt|;
if|if
condition|(
...
name|path
argument_list|,
literal|"Invalid integer discriminator value '"
operator|+
name|valueString
argument_list|)
expr_stmt|;
