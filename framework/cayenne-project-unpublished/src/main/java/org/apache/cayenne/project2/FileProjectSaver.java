...
operator|=
operator|new
name|ConfigurationSaveableGetter
argument_list|()
expr_stmt|;
name|fileEncoding
operator|=
...
argument_list|)
expr_stmt|;
comment|// some configuration nodes don't require their own files to be saved, so check
comment|// for that first; still return such units as we may need to delete the old files.
if|if
condition|(
operator|!
name|node
